...
:::warning 替换组件库
如果你不喜欢 Vant ，或者你想要将老项目迁移到 Fantastic-mobile 上，但老项目里使用的组件库并不是 Vant ，框架也提供了[其他 UI 组件库](/guide/other-ui)的支持。
:::
...
内建组件存放在 `packages/components/src/` 目录下，并通过 `@fantastic-mobile/components` 及对应 resolver 提供自动引入。
